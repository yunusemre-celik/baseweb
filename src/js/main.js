...
// Kullanıcının seçtiği dili hafızadan al, yoksa 'tr' kullan
let currentLang = localStorage.getItem('siteLang') || 'tr';

// Tıklama ile dili değiştirme fonksiyonu (HTML'den çağrılır)
window.changeLanguage = function(lang) {
  if (currentLang === lang) return; // Zaten aynı dilse işlem yapma
  currentLang = lang;
  ...
  loadContent(); // Seçili dile göre JSON'u tekrar çek
};
...
function renderPage(data) {
  ...
  if (socialContainer && data.social) {
    socialContainer.innerHTML = ''; // Dil değiştiğinde çoğalmayı önler
    const socialIcons = {
      ...
      googleMaps: `<svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>`
    };

    if(data.social.instagram) socialContainer.innerHTML += `<a href="${data.social.instagram}" target="_blank" class="text-text-muted hover:text-[#E1306C] transition-colors p-2 bg-white rounded-full shadow-sm hover:shadow-md border border-gray-100">${socialIcons.instagram}</a>`;
    if(data.social.facebook) socialContainer.innerHTML += `<a href="${data.social.facebook}" target="_blank" class="text-text-muted hover:text-[#1877F2] transition-colors p-2 bg-white rounded-full shadow-sm hover:shadow-md border border-gray-100">${socialIcons.facebook}</a>`;
    if(data.social.googleMaps) socialContainer.innerHTML += `<a href="${data.social.googleMaps}" target="_blank" class="text-text-muted hover:text-[#EA4335] transition-colors p-2 bg-white rounded-full shadow-sm hover:shadow-md border border-gray-100">${socialIcons.googleMaps}</a>`;
  }
